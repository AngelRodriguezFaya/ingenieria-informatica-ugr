...
/*
	PROGRAMA 23(V1): Calcula el producto de dos matrices que se leen desde el 
	teclado por el método tradicional de cout y cin.
...
int main(){
	...
	// Entrada de datos.
		...
		// Matriz izquierda.
	cout << " Introduce el numero de filas y columnas de la matriz IZQUIERDA -> ";
	cin >> util_fil_izda; 
	cin >> util_col_izda;
	cout << " Ahora, introduce la matriz POR FILAS: " << endl;
	for (int i = 0; i < util_fil_izda; i++){
		cout << "   ";
		for (int j = 0; j < util_col_izda; j++)
			cin >> matr_izda[i][j];	
	}
	cout << "\n  Matriz izquierda guardada! \n\n" << endl;
	
		// Matriz derecha.
	cout << " Introduce el numero de filas y columnas de la matriz DERECHA -> ";
	cin >> util_fil_dcha; 
	cin >> util_col_dcha;
	cout << " Ahora, introduce la matriz POR FILAS: " << endl;
